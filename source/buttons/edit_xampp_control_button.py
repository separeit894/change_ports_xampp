...
def edit_xampp_control_button(root, style):
    if ctypes.windll.shell32.IsUserAnAdmin():
        ...
        enter_pole_apache = ttk.Entry(
            Frame_Apache, 
            width=20, 
            font=("Arial", 16)
            )
        
        enter_pole_apache.pack(
            side=tk.LEFT
            )
        
        # Значение, которое пользователь ввел ранее
        enter_pole_apache.insert(
            0, 
            str(result_port_apache)
            )
        ...
        Label_ApacheSSL.pack(
            side=tk.LEFT
            )

        # Поле ввода, прикрепляется к окну window, будет иметь шрифт Arial 16 пунктов
        enter_pole_apachessl = ttk.Entry(
            Frame_ApacheSSL, 
            width=20, 
            font=("Arial", 16)
            )
        ...
        enter_pole_apachessl.pack(
            side=tk.LEFT
            )
        
        enter_pole_apachessl.insert(
            0, 
            str(result_port_apachessl)
            )
        ...
        enter_pole_mysql = ttk.Entry(
            Frame_MySQL, 
            width=20, 
            font=("Arial", 16)
            )
        
        enter_pole_mysql.pack(
            side=tk.LEFT
            )
        
        enter_pole_mysql.insert(
            0, 
            str(result_port_mysql)
            )
